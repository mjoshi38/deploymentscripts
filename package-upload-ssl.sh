...
curl -v -s -k --insecure -u $USERNAME:"${PASSWORD}" -F file=@"${PACKAGE_FILE}" -F name="${PACKAGE_NAME}" -F force=true -F install=true -F recursive=true https://$PUBLIC_IP:$PORT/crx/packmgr/service.jsp
